...
class TetrisEnv:
    ...
    def launch_tile(self):
        ...
        if out_of_bounds_at_put:
            ...
            self.current_tile_occupied_cells_in_field = (
                self._get_current_tile_occupied_cells_in_field()
            )
    ...
    def drop_current_tile(self):
        ...
        self.current_tile_occupied_cells_in_field[0] += np.ones(
            shape=(len(self.current_tile_occupied_cells_in_field[0]),), dtype=np.int8
        )

        # revert element at index 0 from np.array to list
        self.current_tile_occupied_cells_in_field[0] = (
            self.current_tile_occupied_cells_in_field[0].tolist()
        )
        ...
        self.current_tile_positionInField[0] = self.current_tile_positionInField[
            0
        ].tolist()
    ...
    def remove_full_rows(self, full_rows_indices: Union[list, np.ndarray]) -> int:
        ...
        self.current_tile_positionInField[0] = [
            row + full_rows_n for row in self.current_tile_positionInField[0]
        ]
    ...
    def rotate(self):
        ...
        self.current_tile[1] = self._rotate_tile(
            tile_to_rotate=current_tile_not_rotated_yet
        )
        self.current_tile[2] = self._update_rotation_value()
        self.current_tile_positionInField = (
            self._current_tile_positionInField_after_rotation(
                tile_to_rotate=current_tile_not_rotated_yet
            )
        )
        self.current_tile_occupied_cells_in_field = (
    ...
    def _current_tile_positionInField_after_rotation(
        self, tile_to_rotate: np.ndarray
    ) -> List[List[int]]:
        """
    ...
    def _current_tile_occupied_cells_in_field_after_rotation(
        self, tile_to_rotate: np.ndarray
    ) -> List[List[int]]:
        current_tile_rotated = self._rotate_tile(tile_to_rotate=tile_to_rotate)
    ...
    def _out_of_bounds_with_rotation(self) -> bool:
        ...
        current_tile_occupied_cells_after_rotation = (
            self._current_tile_occupied_cells_in_field_after_rotation(
                tile_to_rotate=self.current_tile[1]
            )
        )
